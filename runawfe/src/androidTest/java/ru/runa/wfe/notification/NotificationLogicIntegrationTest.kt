...
class NotificationLogicIntegrationTest {
    ...
    private val mockedNotificationHelpers = mockk<NotificationHelpers>()
    ...
    fun severalNewTasks() = runTest {
        ...
        verify (timeout = 2_000, exactly = 1) { mockedNotificationHelpers.showNotification(any(), any(), any())}
    }
    ...
    fun severalNewChatMessages() = runTest {
        ...
        val messages1 = List(5){ MessageAddedBroadcast(author = Actor(name = "testAuthor1"), text="test msg 1") }
        successfulResponseGenerator(jacksonObjectMapper.writeValueAsString(messages1))
        val messages2 = List(1){ MessageAddedBroadcast(author = Actor(name = "testAuthor2"), text="test msg 2") }
    ...
    fun oneNewChatMessage() = runTest {
        ...
        val jsonRooms = jacksonObjectMapper.writeValueAsString(rooms)
        successfulResponseGenerator(jsonRooms)
        val messages1 = List(5){ MessageAddedBroadcast(author = Actor(name = "testAuthor"), text="test msg") }
        successfulResponseGenerator(jacksonObjectMapper.writeValueAsString(messages1))
